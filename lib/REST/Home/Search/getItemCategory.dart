...
Future<List<BlocSize>> getItemsCategory(int id, {int offset, int limit, int type})async {
  List<BlocSize> list = [
  ...
  String url = "http://194.226.171.139:14880/apitest.php/getitemcategory?id=$id";
  ...
  if (response.statusCode == 200) {
    ...
    if (decData['code'] == 200) {
      for (int i = 0; i < decData['mess'].length; i++) {
        if (decData['mess'][i]["blocksize"] == "1") {
          list.add(Category.fromJson(decData['mess'][i]));
        } else if (decData['mess'][i]["blocksize"] == "2") {
          print("================");
          list.add(ProductShort.fromJson(decData['mess'][i]));
        }
      }
    }
    else return [];
